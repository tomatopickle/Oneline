...
export default {
        ...
        data: {
          ...
          likeEmoji: {
            _data: {
              ...
              sheet_y: 28,
              search:
                "+1,thumbsup,thumbs,up,sign,thumbs_up,yes,awesome,good,agree,accept,cool,hand,like",
              skin_tone: 1,
            },
            _skins: null,
            _sanitized: {
  ...
  mounted() {
    ...
    if (this.$route.query?.story && this.$route.query?.user) {
      this.openShortWithId(
        this.$route.query.story,
        this.$route.query.user,
        true
      );
    }
    onValue(
      child(ref(db), `users/${localStorage.getItem("id")}`),
      (snapshot) => {
        ...
        onDisconnect(ref(db, `users/${this.user.id}/lastOnline`)).set(
          serverTimestamp()
        );
        onDisconnect(child(ref(db), `status/${this.user.id}`)).update({
          status: "offline",
        });
        this.loading = false;
        this.getChats();
        ...
        if (
          Object.keys(data?.chats).length !=
          Object.keys(this.user?.chats).length
        ) {
  ...
  methods: {
    ...
    createTagFunction() {
      set(
        child(
          ref(db),
          `chats/${this.chat.id}/tags/${this.createTag.data.name}`
        ),
        this.createTag.data
      );
    ...
    assignTag(tagName, userId) {
      set(
        child(ref(db), `chats/${this.chat.id}/assignedTags/${userId}`),
        tagName
      );
    },
    ...
    clearCache() {
      if (
        !confirm(
          "You will be logged out after clearing cache, do you want to proceed?"
        )
      )
        return;
    ...
    filePastedInMsgBar(fileObj) {
      ...
      uploadBytes(
        storageRef(storage, `messages/${id}_${fileObj.name}`),
        fileObj
      ).then((snapshot) => {
        getDownloadURL(snapshot.metadata.ref).then((url) => {
          file.url = url;
    ...
    startMeetingWithUser(user) {
      for (var chatId in this.chats) {
        ...
        if (chat.type == "personal" && chat.data.id == user.id) {
          ...
          axios
            .post(
              "https://Oneline-Functions.abaanshanid.repl.co/meetings/rooms"
            )
            .then((response) => {
              const room = response.data.room;
              const token = response.data.token;
              console.log(response.data);
              set(ref(db, "meetingInvite/" + chat.id), {
                from: this.user,
                room,
                chat: chat,
              });
              const data = router.resolve({
                path: "/meeting",
                query: { meetingId: room.url, token },
              });
              window.open(data.href, "_blank");
              setTimeout(() => {
                remove(ref(db, "meetingInvite/" + chat.id));
              }, 3000);
              this.newMeetingBtnDisabled = false;
              if (
                this.settings.notificationGranted &&
                this.settings.data.notification.enabled &&
                this.settings.data.notification.meetingNotifcations
              ) {
                if (chat.type == "personal") {
                  new Notification(`${chat.name} has been invited`, {
                    badge:
                      "https://res.cloudinary.com/abaan/image/upload/v1642608838/pngaaa.com-463061_lry8is.png",
                  });
                } else {
                  new Notification(
                    `Everyone in ${chat.name} has been invited`,
                    {
                      badge:
                        "https://res.cloudinary.com/abaan/image/upload/v1642608838/pngaaa.com-463061_lry8is.png",
                    }
    ...
    likeShort() {
      for (var chatId in this.chats) {
        ...
        if (chat.type == "personal" && chat.data.id == this.shorts.user.id) {
          ...
          update(child(ref(db), `messages/${chat.id}/${Date.now()}`), {
            type: "likeShort",
            short: this.shorts.short,
    ...
    viewedShort() {
      ...
      this.shorts.short = this.getByIndex(
        this.shorts.shorts,
        this.shorts.index
      );
      ...
      if (this.shorts.short) {
        set(
          ref(db, `users/${this.user.id}/lastSeenShort/${this.shorts.user.id}`),
    ...
    uploadGroupAvatar() {
      ...
      input.onchange = (e) => {
        ...
        console.log(file);
        uploadBytes(
          storageRef(storage, `groupAvatar/${this.chat.id}`),
          file
        ).then((snapshot) => {
          ...
          getDownloadURL(snapshot.metadata.ref).then((url) => {
            axios
              .post(
                "https://oneline-functions.abaanshanid.repl.co/group/update/avatar",
                { id: this.chat.id, avatar: url }
              )
              .then((response) => {
                // handle success
                ...
                update(
                  child(ref(db), `messages/${this.chat.id}/${Date.now()}`),
                  {
                    text: `${this.user.username} changed the group image`,
                    type: "info",
                    time: Date.now(),
                    sender: this.user.id,
                    action: "groupInfoChanged",
                  }
                );
    ...
    uploadAvatar() {
      ...
      input.onchange = (e) => {
        ...
        console.log(file);
        uploadBytes(
          storageRef(storage, `userAvatar/${this.user.id}`),
          file
        ).then((snapshot) => {
          console.log("Uploaded a blob or file!");
          console.log(snapshot);
    ...
    startMeeting() {
      ...
      axios
        .post("https://Oneline-Functions.abaanshanid.repl.co/meetings/rooms")
        .then((response) => {
          const room = response.data.room;
          const token = response.data.token;
          console.log(response.data);
          ...
          if (
            ...
            if (chat.type == "personal") {
              ...
              new Notification(`Everyone in ${chat.name} has been invited`, {
                badge:
                  "https://res.cloudinary.com/abaan/image/upload/v1642608838/pngaaa.com-463061_lry8is.png",
              });
            }
    ...
    async sendRecording() {
      ...
      console.log(file);
      const time = Date.now();
      uploadBytes(storageRef(storage, `messages/${time}`), file).then(
        (snapshot) => {
          console.log("Uploaded a blob or file!");
          console.log(snapshot);
          getDownloadURL(snapshot.metadata.ref).then((url) => {
    ...
    getGifs() {
      ...
      this.gif.search = "";
      onValue(
        query(ref(db, `recent-gifs/${this.user.id}`), limitToLast(4)),
        (snapshot) => {
          if (snapshot.exists()) {
            this.gif.recent = JSON.parse(
          ...
          axios
            .get(
              `https://api.giphy.com/v1/gifs/trending?rating=g&api_key=${process.env.VUE_APP_GIPHY_KEY}`,
              {}
            )
            .then((response) => {
              ...
              axios
                .get(
                  `https://api.giphy.com/v1/trending/searches?api_key=${process.env.VUE_APP_GIPHY_KEY}`,
                  {}
                )
                .then((chips) => {
    ...
    getAllRecentGifs() {
      ...
      this.gif.loading = true;
      onValue(
        query(ref(db, `recent-gifs/${this.user.id}`), limitToLast(25)),
        (snapshot) => {
          if (snapshot.exists()) {
            this.gif.recent = JSON.parse(
              stringify(snapshot.val(), function (a, b) {
    ...
    searchGifs(e, q) {
      ...
      axios
        .get(
          `https://api.giphy.com/v1/gifs/search?rating=g&api_key=${process.env.VUE_APP_GIPHY_KEY}&q=${this.gif.search}`,
          {}
        )
        .then((response) => {
          this.gif.gifs = response.data.data;
    ...
    sendGif(gif) {
      ...
      update(child(ref(db), `messages/${this.chat.id}/${Date.now()}`), {
    ...
    changeGroupInfo() {
      ...
      axios
        .post("https://oneline-functions.abaanshanid.repl.co/group/update", {
          id: this.chat.id,
          name: this.groupInfo.data.name,
          description: this.groupInfo.data.description,
        })
        .then((response) => {
          // handle success
          update(child(ref(db), `messages/${this.chat.id}/${Date.now()}`), {
            text: `${this.user.username} changed the group's information`,
            type: "info",
            time: Date.now(),
            sender: this.user.id,
            action: "groupInfoChanged",
    ...
    sendHi() {
      update(child(ref(db), `messages/${this.chat.id}/${Date.now()}`), {
        text: emojiConvertor.replace_colons(":wave: Hi"),
        sender: this.user.id,
        time: Date.now(),
      });
    },
    ...
    openChat(chat, controlled) {
      ...
      onValue(query(ref(db, `chats/${chat.id}`)), (snapshot) => {
        ...
        onValue(
          query(ref(db, `messages/${chat.id}`), limitToLast(this.limit)),
          (snapshot) => {
    ...
    getGroupChat(chatId, chat) {
      ...
      onValue(
        query(ref(db, `lastSeenMessage/${this.user.id}/${chatId}`)),
        (snapshot) => {
          ...
          onValue(
            query(
              ...
              orderByKey(),
              startAfter(lastSeenMessageTime),
              limitToLast(100)
            ),
            (snapshot) => {
              off(
              ...
              if (this.chats[chatId]) {
                this.chats[chatId].unreadMessages = Object.keys(
                  snapshot.val()
                ).length;
              } else {
        ...
        (data) => {
          ...
          get(ref(db, `users/${lastMessage?.sender}`)).then((snapshot) => {
            ...
            if (
              this.settings.notificationGranted &&
              this.settings.data.notification.enabled &&
              this.settings.data.notification.newMessage
            ) {
              if (this.windowHidden && this.user.id != lastMessage?.sender) {
                new Notification(`${sender.username} in ${chat.name}`, {
                  body: this.stripHtml(lastMessage.text),
                });
              }
            }
          });
      ...
      onValue(
        query(ref(db, `messages/${chatId}`), limitToLast(1)),
        (snapshot) => {
          if (snapshot.exists()) {
          ...
          get(ref(db, `users/${lastMessage?.sender}`)).then((snapshot) => {
            ...
            this.chats[chatId] = {
              ...
              addedTime: chat.addedTime,
              unreadMessages,
              lastMessage: lastMessage
                ? lastMessage
                : { text: "New Group", time: Date.now() },
            };
            var s = stringify(
              JSON.parse(JSON.stringify(this.chats)),
              function (a, b) {
                return a.value.lastMessage?.time < b.value.lastMessage?.time
                  ? 1
                  : -1;
              }
            );
        ...
        (data) => {
          ...
          get(ref(db, `users/${lastMessage?.sender}`)).then((snapshot) => {
            ...
            if (
              ...
              if (this.windowHidden && this.user.id != lastMessage?.sender) {
                new Notification(`${sender.username} in DMs`, {
                  body: this.stripHtml(lastMessage.text),
                });
              }
            }
              ...
              onValue(query(ref(db, `status/${usr}`)), (snapshot) => {
                ...
                onValue(
                  ...
                  (snapshot) => {
                    ...
                    get(ref(db, `users/${lastMessage?.sender}`)).then(
                      (snapshot) => {
                        if (lastMessage) {
                          var sender = snapshot.val();
                          lastMessage.senderInfo = sender;
                        ...
                        this.chats[chatId] = {
                          ...
                          unreadMessages,
                          data,
                          lastMessage: snapshot.exists()
                            ? lastMessage
                            : { text: "New Chat", time: Date.now() },
                          status,
                        };
                        var s = stringify(
                          JSON.parse(JSON.stringify(this.chats)),
                          function (a, b) {
                            return a.value.lastMessage?.time <
                              b.value.lastMessage?.time
                              ? 1
                              : -1;
                          }
                        );
    ...
    userSeenLastMessage() {
      const lastSeenMsgTime = Object.keys(this.messages)[
      ...
      set(
        child(ref(db), `lastSeenMessage/${this.user.id}/${this.chat.id}`),
        lastSeenMsgTime
      );
    },
    getShorts(user) {
      ...
      onValue(
        query(
          ...
          orderByKey(),
          startAfter(time.toString())
        ),
        (snapshot) => {
          if (!snapshot.exists()) return;
          this.shortsAvatars[user.id] = {
            user,
            badge: Object.keys(snapshot.val()).length,
          };
        }
      );
    },
    votePoll(option) {
      set(
        child(
          ref(db),
          `shorts/${this.shorts.user.id}/${this.shorts.short.time}/poll/options/${option.name}/voters/${this.user.id}`
        ),
        this.user
      );
      set(
        child(
          ref(db),
          `shorts/${this.shorts.user.id}/${this.shorts.short.time}/poll/options/${option.name}/votes`
        ),
        option.votes++
      );
      set(
        child(
          ref(db),
          `shorts/${this.shorts.user.id}/${this.shorts.short.time}/voters/${this.user.id}`
        ),
        this.user
      );
      get(
        child(
          ref(db),
          `shorts/${this.shorts.user.id}/${this.shorts.short.time}`
        )
      ).then((snapshot) => {
    ...
    openShort(short) {
      ...
      onValue(
        query(ref(db, `shorts/${short.user.id}`), limitToLast(short.badge)),
        (snapshot) => {
          this.shorts.user = short.user;
          this.shorts.shorts = JSON.parse(
            stringify(snapshot.val(), function (a, b) {
              return a.key > b.key ? 1 : -1;
            })
          );
          ...
          set(
            ref(
              db,
              `users/${this.user.id}/lastSeenShort/${this.shorts.user.id}`
            ),
            this.shorts.short.time
          );
    ...
    openShortWithId(id, userId, controlled) {
      ...
      get(child(ref(db), `users/${userId}`)).then((snapshot) => {
        ...
        get(child(ref(db), `shorts/${userId}/${id}`)).then((snapshot) => {
    ...
    getMessagePreview(chat) {
      if (!chat.lastMessage?.senderInfo) {
        return `New Chat`;
      }
    ...
    sendReplyMessage() {
      ...
      update(child(ref(db), `messages/${this.chat.id}/${Date.now()}`), {
        text: emojiConvertor.replace_colons(this.message.text),
        type: "reply",
        replyingTo: this.reply.message,
        sender: this.user.id,
        time: Date.now(),
      });
      this.message.text = "";
      ...
      onValue(
        ...
        (snapshot) => {
          ...
          if (snapshot.exists()) {
            ...
            update(child(ref(db), `users/${this.user.id}/chats`), {
              [chatId]: chat,
            });
    ...
    joinGroup() {
      get(child(ref(db), "chats/" + this.newChat.data.group.id)).then(
        (snapshot) => {
          if (snapshot.exists()) {
            var data = snapshot.val();
            ...
            update(child(ref(db), `chats/${this.newChat.data.group.id}`), {
              members,
            });
    ...
    archiveGroupFunction() {
      ...
      update(child(ref(db), `messages/${this.chat.id}/${Date.now()}`), {
        text: `The admin (${this.user.username}) archived the group`,
        type: "info",
        time: Date.now(),
        sender: this.user.id,
        action: "groupArchived",
      });
    },
    unarchiveGroupFunction() {
      set(child(ref(db), `chats/${this.chat.id}/archive`), false);
      update(child(ref(db), `messages/${this.chat.id}/${Date.now()}`), {
        text: `The admin (${this.user.username}) unarchived the group`,
        type: "info",
        time: Date.now(),
        sender: this.user.id,
        action: "groupUnarchived",
      });
    },
    leaveGroupFunction() {
      this.leavingGroup = true;
      axios
        .post("https://oneline-functions.abaanshanid.repl.co/group/leave", {
          id: this.chat.id,
          user: this.user.id,
        })
        .then((response) => {
          // handle success
          update(child(ref(db), `messages/${this.chat.id}/${Date.now()}`), {
            text: `${this.user.username} left the group`,
            type: "info",
            time: Date.now(),
            sender: this.user.id,
            action: "groupLeft",
    ...
    sendMessage() {
      if (
        document.querySelector("#messageInp .editable").textContent.trim()
          .length < 1
      )
        return;
    ...
    checkEnterKey(e) {
      ...
      if (
        ...
        this.searchEmojisForCompletion(emojiQuery);
        if (
          e.target.innerText.slice(0, -1).replace(emojiQuery, "").slice(-1) !=
          ":"
        ) {
          this.clearEmojiSearch();
        }
        ...
        if (
          !this.isScrolledIntoView(
            document.querySelector("#emojiComplete .selected"),
            document.getElementById("emojiComplete")
          )
        ) {
    ...
    checkIfScrolledToTop(e) {
      ...
      this.scrollDownBtn =
        e.target.scrollTop + 75 < e.target.scrollHeight - e.target.offsetHeight;
    },
    checkIfUserTyping(e) {
      update(child(ref(db), `typing/${this.user.id}`), {
        chat: this.chat.id,
        typing: true,
        user: this.user,
      });
